...
Page({
  data: {
    ...
    activeTab: 0,
    url: encodeURIComponent('https://wx.sharkmeida.cn/dist/index.html#/4pageEdit?id='),
    refund_type: 0,
    refundloc_orderid: '',
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          wx.request({
            //项目的真正接口，通过字符串拼接方式实现
            url: "https://wx.sharkmeida.cn/groupon/delete",
            header: {
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          wx.request({
            //项目的真正接口，通过字符串拼接方式实现
            url: "https://wx.sharkmeida.cn/groupon/copy",
            header: {
              "content-type": "application/json;charset=UTF-8"
            },
