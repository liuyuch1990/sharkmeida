...
Page({
  data: {
    ...
    activeTab: 0,
    url: encodeURIComponent('https://wx.sharkmeida.cn/dist/index.html#/3pageEdit?id='),
    refund_type: 0,
    refundloc_orderid: '',
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          wx.request({
            //项目的真正接口，通过字符串拼接方式实现
            url: "https://wx.sharkmeida.cn/bargin/delete",
            header: {
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          wx.request({
            //项目的真正接口，通过字符串拼接方式实现
            url: "https://wx.sharkmeida.cn/bargin/copy",
            header: {
              "content-type": "application/json;charset=UTF-8"
            },
